...
static void* aio_write_thread(void* arg) {
    ...
    ssize_t result = write(aiocbp->aio_fildes, (const void*)aiocbp->aio_buf, aiocbp->aio_nbytes);
...
int aio_write(struct aiocb *aiocbp) {
    ...
    if (pthread_create(&aiocbp->_aio_thread, NULL, aio_write_thread, aiocbp) != 0) {
        aiocbp->_aio_state = 2;
        aiocbp->_aio_error = errno;
        errno = EAGAIN;
        return -1;
