...
int
posix_spawnp (pid_t * __restrict __pid,
	      ...
	      char *const argv[], char *const __envp[])
{
	  ...
	  if (si.lpAttributeList)
	    {
	      ...
	      fullPathSize = 0;
	      fullPath = (char *) &fullPathSize;
	      fullPathSize =
		SearchPathA (NULL, __path, NULL, 1, fullPath, NULL);
	      if (!fullPathSize)
		{
		...
		{
		  fullPath =
		    (char *) malloc (fullPathSize * sizeof (*fullPath));
		  if (fullPath)
		    {
		      fullPathSize =
			SearchPathA (NULL, __path, NULL, fullPathSize,
				     fullPath, NULL);
		      if (!fullPathSize)
			{
			  free (fullPath);
		  ...
		  && !(__envp && !environment)
		  && CreateProcess (fullPath, commandLine, 0, 0, TRUE,
  ...
  if (!si.lpAttributeList)
	...
	{
	  pi.hProcess =
	    (HANDLE) _spawnlpe (_P_NOWAIT, __path, (const char *) commandLine,
				0, (const char *const *) __envp);
	}
      else
	{
	  pi.hProcess =
	    (HANDLE) _spawnlpe (_P_NOWAIT, __path, 0,
      ...
      if (pi.hProcess == INVALID_HANDLE_VALUE)
	{
	  ...
	  if (fail == E2BIG || value == E2BIG)
	      ...
	      else
		{
	      ...
	      if (!(argv && *argv && !commandLine)
		  && !(__envp && !environment)
		  && CreateProcess (fullPath, commandLine, 0, 0, TRUE, 0,
